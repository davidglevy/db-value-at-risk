...
from var_support.var_helper import getTaskInfo, setup_mlflow
...
run_date = dt.strptime(config['model_training_date'], '%Y-%m-%d')

# COMMAND ----------

market_df = spark.table(config['volatility_table']).filter(col('date') < lit(run_date)).select('date', 'features')
...
stocks_df = get_stock_returns().filter(col('date') < lit(run_date))
...
# MAGIC # Use Our Model
# MAGIC
# MAGIC We will now use our simple model which shows how easy it is to train a model and use it within a pipeline
...
plt.title("Model Sum of Mean Square for each instrument (WSSE)")
plt.xticks(rotation=45)
plt.ylabel("Sum of Mean Square")
